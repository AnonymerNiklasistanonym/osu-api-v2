...
export const userStatisticsObjectDeprecatedKeys = [
    // Not documented
    "rank",
    "global_rank_exp",
    "pp_exp",
...
export const checkUserStatisticsObject = (
    ...
    // List of all keys that will be checked
    const checkedKeys: string[] = [...userStatisticsObjectDeprecatedKeys]

    checkedKeys.push("count_100")
    expect(userStatistics.count_100).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("count_300")
    expect(userStatistics.count_300).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("count_50")
    expect(userStatistics.count_50).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("count_miss")
    expect(userStatistics.count_miss).to.be.a("number").greaterThanOrEqual(0)

    if (userStatistics.country_rank !== undefined) {
        if (userStatistics.country_rank != null) {
            checkedKeys.push("country_rank")
            expect(userStatistics.country_rank)
                .to.be.a("number")
                .greaterThanOrEqual(0)
        } else {
            checkedKeys.push("country_rank")
            expect(userStatistics.country_rank).to.be.null
        }
    }

    if (userStatistics.global_rank !== undefined) {
        if (userStatistics.global_rank != null) {
            checkedKeys.push("global_rank")
            expect(userStatistics.global_rank)
                .to.be.a("number")
                .greaterThanOrEqual(0)
        } else {
            checkedKeys.push("global_rank")
            expect(userStatistics.global_rank).to.be.null
        }
    }

    checkedKeys.push("grade_counts")
    checkUserCompactStatisticsGradeCountsObject(userStatistics.grade_counts)

    checkedKeys.push("hit_accuracy")
    expect(userStatistics.hit_accuracy).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("is_ranked")
    expect(userStatistics.is_ranked).to.be.a("boolean")

    checkedKeys.push("level")
    checkUserStatisticsLevelObject(userStatistics.level)

    checkedKeys.push("maximum_combo")
    expect(userStatistics.maximum_combo).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("play_count")
    expect(userStatistics.play_count).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("play_time")
    expect(userStatistics.play_time).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("pp")
    expect(userStatistics.pp).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("ranked_score")
    expect(userStatistics.ranked_score).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("replays_watched_by_others")
    expect(userStatistics.replays_watched_by_others)
        .to.be.a("number")
        .greaterThanOrEqual(0)

    checkedKeys.push("total_hits")
    expect(userStatistics.total_hits).to.be.a("number").greaterThanOrEqual(0)

    checkedKeys.push("total_score")
    expect(userStatistics.total_score).to.be.a("number").greaterThanOrEqual(0)

    if (userStatistics.variants !== undefined) {
        checkedKeys.push("variants")
        expect(userStatistics.variants).to.be.an("array")
        for (const element of userStatistics.variants) {
            checkUserGameModeVariantObject(element)
        }
    }

    genericCheckObjectForUncheckedKeys(userStatistics, checkedKeys)
}
...
export const checkUserRankHighestObject = (
    userRankHighest: Readonly<UserRankHighest>,
): void => {
    const objectInfo = () =>
        `UserRankHighest: ${JSON.stringify(userRankHighest)}`
    expect(userRankHighest).to.be.an("object")

    // List of all keys that will be checked
    const checkedKeys: string[] = []

    checkedKeys.push("rank")
    expect(userRankHighest.rank).to.be.a("number").greaterThan(0, objectInfo())

    checkedKeys.push("updated_at")
    expect(userRankHighest.updated_at)
        .to.be.a("string")
        .with.a.lengthOf.greaterThan(0, objectInfo())

    genericCheckObjectForUncheckedKeys(userRankHighest, checkedKeys)
}
...
export const checkUserObject = (
    ...
    expect(user.rank_highest).to.not.be.undefined
    expect(user.rank_history).to.not.be.undefined
...
export const userCompactObjectUndocumentedKeys = [
    // Deprecated: 2021-04-20 use cover.url
    "cover_url",
    // Deprecated: 2021-06-09 eplaced with ranked_beatmapset_count/pending_beatmapset_count
...
export const checkUserCompactObject = (
    ...
    // List of all keys that will be checked
    const checkedKeys = [...userCompactObjectUndocumentedKeys]
    ...
    if (user.rank_highest !== undefined) {
        checkedKeys.push("rank_highest")
        checkUserRankHighestObject(user.rank_highest)
    }
