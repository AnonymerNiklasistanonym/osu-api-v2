 ...
 */
export const timeoutForRequestsInMs = (requests: number) => 2 * 2000 * requests
...
export const saveResponse = async <DATA_TYPE extends unknown>(
    prefix: string,
    name: string,
    jsonData: DATA_TYPE,
): Promise<void> => {
    ...
    await fs.mkdir(cachedOsuApiResponses, {
        recursive: true,
    })
    await fs.writeFile(outputFile, JSON.stringify(jsonData), {
        encoding: "utf8",
    })
}
